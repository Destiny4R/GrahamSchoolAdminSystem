...
{
    public class pta_payment_receiptModel : PageModel
    {
        private readonly IUnitOfWork _unitOfWork;

        public PaymentInvoiceViewModel Invoice { get; set; } = new();

        public pta_payment_receiptModel(IUnitOfWork unitOfWork)
        {
        ...
        public async Task<IActionResult> OnGetAsync(int paymentId)
        {
            try
            {
                ...
                var payment = await _unitOfWork.PTAPaymentServices.GetPTAPaymentByIdAsync(paymentId);
        ...
        {
            var totalPreviousPayments = await _unitOfWork.PTAPaymentServices.GetTotalPreviousPTAPaymentsAsync(payment.TermRegId, payment.Id);
            ...
            return new PaymentInvoiceViewModel
            {
                ...
                PreviousBalance = await _unitOfWork.PTAPaymentServices.GetPreviousBalanceAsync(payment.TermRegId),
        ...
        private string GenerateStandaloneHtmlInvoice(PaymentInvoiceViewModel invoice)
        {
            ...
            html.AppendLine("                <p>PTA Payment Management System</p>");
            html.AppendLine("            </div>");
            ...
            // PTA Payment Summary
            html.AppendLine("        <div class=\"invoice-section\">");
            html.AppendLine("            <h3>📋 PTA Payment Summary</h3>");
            html.AppendLine("            <div class=\"payment-summary\">");
            html.AppendLine("                <div class=\"summary-item label\">");
            html.AppendLine("                    <span>PTA Fee Structure & Payments</span>");
            html.AppendLine("                </div>");
            html.AppendLine("                <div class=\"summary-item\">");
            html.AppendLine("                    <span>Total PTA Fees:</span>");
            ...
            html.AppendLine($"                        <span><span class=\"payment-state-badge {invoice.PaymentState.ToLower()}\">{invoice.PaymentState}</span></span>");
            html.AppendLine("                    </div>");
            html.AppendLine("                </div>");
            html.AppendLine("            </div>");
            html.AppendLine("        </div>");
        ...
        * {
        ...
        .payment-state-badge.partpayment {
            background-color: #fff3cd;
            color: #856404;
        }

        .payment-state-badge.completed {
            background-color: #d4edda;
            color: #155724;
        }

        .payment-state-badge.cancelled {
            background-color: #f8d7da;
            color: #721c24;
        }
        ...
        /// </summary>
        private string GeneratePaymentReference(int paymentId, int termRegId)
        {
            return $"PAY-{DateTime.Now:yyyyMMdd}-{termRegId:D4}-{paymentId:D4}";
        }
    }
}
